...
4. **Physical text removal verification** (proves not just overlay):
   - Open the exported redacted PDF in a PDF reader
   - Try to select text where email/phone were - selection should fail (no text to select)
   - Alternatively use `pdftotext output.pdf -` and verify redacted values are NOT in text output
   - This proves `apply_redactions()` physically removed text, not just covered it with white rectangle
</verification>
