...
must_haves:
  truths:
    ...
    - "usage_daily table stores aggregated daily totals per project"
    - "SQLite trigger auto-aggregates events into daily table on insert"
    - "projects table has is_pinned column for quick-access sidebar"
  artifacts:
...
<objective>
Add database schema for usage tracking (token counts, API calls) and quick-access project pinning.

Purpose: Establishes the data layer for tracking LLM usage costs and enabling project drag-drop to sidebar.
Output: Migration v3 with usage_events, usage_daily tables, trigger, and is_pinned column.
</objective>
...
<task type="auto">
  ...
  <action>
    ...
    Also in this migration, add is_pinned column to projects table:
    ```sql
    ALTER TABLE projects ADD COLUMN is_pinned INTEGER NOT NULL DEFAULT 0
    ```
    Check if column exists before adding (same pattern as status column).
  </action>
  <verify>npm run lint --quiet shows no errors in database.ts</verify>
  <done>Migration v3 creates usage tables, trigger, and is_pinned column</done>
</task>

<task type="auto">
  <name>Task 2: Add usage tracking CRUD functions</name>
  <files>src/main/database.ts</files>
  <action>
    ...
    6. getPinnedProjects(): ProjectSummary[]
       - Same query as getAllProjects but WHERE is_pinned = 1
       - Return array of pinned projects
  </action>
  <verify>npm run lint --quiet shows no TypeScript/ESLint errors</verify>
  <done>Six usage tracking functions exported and type-safe</done>
</task>
...
- is_pinned column added to projects table
- All six CRUD functions compile without TypeScript errors
</success_criteria>
