...
must_haves:
  truths:
    ...
    - "User sees section name and statistics when hovering a wedge"
    - "User sees decorative outer ring surrounding the wedges"
...
2. Create `src/renderer/components/wheel/WheelSection.tsx`:
   ...
   - Tooltip on hover: Use React state (hoveredSection) and onMouseEnter/onMouseLeave on the motion.g to track which section is hovered. Render an absolutely positioned div tooltip over the SVG container (not inside SVG — HTML tooltips over SVG are more reliable than SVG-native approaches). Tooltip content shows section name and placeholder stats (e.g., "Pipeline: 0", "Response rate: --"). Position the tooltip near the mouse using a ref or clientX/clientY from the mouse event.
