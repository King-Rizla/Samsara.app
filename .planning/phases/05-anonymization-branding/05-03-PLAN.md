...
<task type="auto">
  <name>Task 2: Create Export modal with mode selection</name>
  <files>
    src/renderer/components/export/ExportModal.tsx
  </files>
  <action>
...
3. UI layout (terminal aesthetic):
   ...
   - For bulk: "Choose folder" button using electron dialog
...
5. Add folder selection for bulk:
   - Use Electron's dialog.showOpenDialog via new IPC handler
   - Add to preload: `selectFolder(): Promise<{ canceled: boolean; path?: string }>`
   - Add to index.ts: IPC handler that calls `dialog.showOpenDialog({ properties: ['openDirectory'] })`
  </action>
  <verify>
1. TypeScript compiles
2. Modal opens when triggered
3. Mode selection works (radio buttons update state)
4. Blind profile toggle works
5. Export button is disabled while exporting (spinner shown)
  </verify>
  <done>
Export modal renders with mode selection, blind profile toggle, and export button. UI matches terminal theme.
  </done>
</task>

<task type="auto">
  <name>Task 3: Wire export button into queue and editor</name>
  <files>
    src/renderer/App.tsx
    src/renderer/stores/queueStore.ts
...
4. Add folder selection IPC (for bulk export):
   - In preload.ts:
   ```typescript
...
5. Load recruiter settings on app start:
...
9. Performance: Export completes in <500ms for typical CV
</verification>
