...
must_haves:
  truths:
    - "Commits with lint errors are automatically blocked before reaching the repo"
    - "Every Claude session starts with a clear safety checklist of 7 verification steps"
    - "Python code is continuously scanned for known vulnerability patterns (Bandit rules)"
    - "Secrets accidentally added to code are caught before commit"
  artifacts:
    - path: "CLAUDE.md"
...
2. Create `pyproject.toml` in repo root with:
   ...
   - `[tool.pytest.ini_options]`: testpaths = ["python-src/tests"], python*files = "test*\*.py"
...
<verify> - `CLAUDE.md` exists and contains "Mandatory Pre-Completion Checks" - `pyproject.toml` exists and contains [tool.ruff] and [tool.mypy] sections - `vitest.config.ts` exists and imports from 'vitest/config'
</verify>
...
10. Run `npx husky` to verify hooks are installed. Run `npx lint-staged --diff="HEAD"` to verify lint-staged config parses correctly.
    ...
    <verify> - `npx husky --version` prints version (Husky installed) - `.husky/pre-commit` exists and contains "lint-staged" - `cat .lintstagedrc.json` shows valid JSON - `.pre-commit-config.yaml` exists with ruff and gitleaks hooks - `.gitleaks.toml` exists - `package.json` contains "test", "typecheck", "prepare" scripts - `python-src/requirements-dev.txt` exists with ruff, mypy, pytest, hypothesis - Verify hook blocks bad commits: create a file with a lint error (e.g., `echo "const x = 1" > test-hook.ts`), stage it (`git add test-hook.ts`), attempt `git commit -m "test"`, verify the hook runs lint-staged and blocks the commit. Clean up: `git reset HEAD test-hook.ts && rm test-hook.ts`
    </verify>
