...
must_haves:
  truths:
    ...
    - "User can select multiple CVs from queue and assign them to a JD for scoring"
...
<task type="auto">
  <name>Task 2: Create JD list, match results, and panel components</name>
  <files>
    src/renderer/components/jd/JDList.tsx
    src/renderer/components/jd/MatchResults.tsx
    src/renderer/components/jd/JDPanel.tsx
...
export function MatchResults() {
  ...
  const { items: queueItems, selectedIds, selectAll, clearSelection } = useQueueStore();
    ...
    <div className="flex flex-col h-full">
      ...
      <div className="p-4 border-b border-border space-y-2">
        ...
        <p className="text-xs text-muted-foreground">
          Select CVs in the queue, then click "Match Selected" to calculate scores.
        </p>
      </div>
...
1. JD input with LLM extraction of requirements
...
5. Skill highlighting in CV editor when viewing against a JD
  </what-built>
  <how-to-verify>
...
2. **Add a Job Description:**
   ...
   - Paste a sample JD text (e.g., from a job posting)
...
4. **Match CVs against JD:**
   - Select the JD in "Job Descriptions" tab
   - Click "Match All CVs" button in "Match Results" tab
...
3. **JD Flow**: Paste -> Extract -> List -> Select -> Match -> Results
4. **Highlighting**: Skills highlighted when viewing CV against selected JD
5. **Match Quality**: Scores and labels match expected behavior

Requirements fulfilled:
- M-01a: User can paste or upload a Job Description (paste implemented)
